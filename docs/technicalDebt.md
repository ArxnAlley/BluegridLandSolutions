...
### 10i. The validator toolchain lives outside the repository — **FIRST PIECE COMMITTED 2026-08-27; THE BULK IS STILL OUTSIDE**

**PARTIALLY ADDRESSED, 2026-08-27.** `_qa/` now exists in the repository and is the first validation infrastructure this project has ever version-controlled. It holds a **range-capable static server**, a **56-check owner-video suite**, a **self-discovering sitewide page regression** (33 pages, asserting no console errors, exactly one `h1`, and that every asset any page requests actually resolves), and a shared harness. `node _qa/runAll.js` runs everything and exits non-zero on failure. **90/90 passing at the time of writing.**

Three deliberate design choices, because this item's history is a history of things rotting:

1. **The page list is discovered, not written down.** A hardcoded list goes stale the day someone adds a page — and during this very session a hand-typed URL produced a "failure" that was a typo in the test, not a defect in the site.
2. **The orientation reference frame is generated by ffmpeg at run time, not committed.** A 170KB raw binary in Git would go stale the moment the video changed and nobody would notice.
3. **The folder is named `_qa`, not `qa`.** This repository *is* the deployed site; Jekyll (which GitHub Pages runs, since there is no `.nojekyll`) does not copy `_`-prefixed directories into the built site. See item 53.

**WHAT IS STILL OUTSIDE, AND IT IS MOST OF IT.** The 28 scratchpad suites — `validateAssets`, `validateSeo`, `validateResponsiveImages`, `validateFooterLegalRow`, the rest — **were not carried into this session and were not run.** They remain uncommitted and are now one lost scratchpad from gone. `validateAssets` in particular is still the only guard against the casing-mismatch 404 described at the bottom of this item, and it still exists nowhere in version control. **This item is not closed.** The precedent is set and the pattern is proven; the remaining work is to bring the rest across the same way.
...
### 13. ~~Chase owner introduction video~~ — **SHIPPED 2026-08-27, uncommitted at time of writing**
The file arrived and the section is live. **The prediction in this item was wrong in an instructive way: the video does not need rotating.** It carries a **-180 degree display matrix** — the pixels are stored inverted and every browser applies the matrix on playback. Correcting it by re-encoding would have inverted Chase *twice*. The matrix is preserved by the stream copy and asserted in QA against an ffmpeg-decoded reference. **Anyone replacing the source must re-check this before assuming a rotation is needed.**

What ships, all in `graphics/videos/`:

| File | Role |
|---|---|
| `chaseIntro.web.mp4` | 8,742,071 B. Lossless `-c copy -movflags +faststart` remux of the master. `moov` at the front. Video and audio bitstreams hash **identically** to the master's. |
| `chaseIntro.poster.webp` | 640x360, 54,972 B, q88. Frame **t=21.30s** — the sharpest his face gets in the whole 29.2s, mouth closed, square to camera. |
| `chaseIntro.en.vtt` | 13 cues, English, transcribed from his own audio. **Attached but NOT defaulted on** pending sign-off. |
| `IntroVideoFromChase.mp4` | The 9MB master. **Gitignored** — see item 52. |

`preload="none"`, so **nothing of the video is fetched until a visitor presses play**. A centred play affordance was added because Chrome desktop draws no middle button of its own and a poster of a man in a field otherwise reads as a photograph; it covers the frame, removes itself on the `play` event, and lives inside the slot that already owns the 16:9 box so it cannot shift layout.

**No markup change was required** — the injector did the whole job, exactly as this item predicted.

**A higher-quality recording is still expected** once better equipment is available. The swap remains a config change, but **the poster and the captions were both derived from this take**: `chaseIntro.en.vtt`'s timestamps are meaningless against a different recording, and the poster is a frame of this one. Replacing the source means regenerating both.
...
**The trade-off is real and should not be forgotten: the one irreplaceable artifact in this project is now the one artifact version control is not looking after.** `chaseIntro.web.mp4` and `chaseIntro.poster.webp` can both be regenerated from the master; the master can be regenerated from nothing.
...
### 54. The caption track is a draft and is deliberately switched off
`graphics/videos/chaseIntro.en.vtt` is attached to the player as `<track kind="captions" srclang="en">` **without `default`**, so it appears in the player's captions menu and is not shown unless a visitor asks for it. That is intentional: the wording has not been approved by Chase, and `default` would paint unapproved text across his face for every visitor.

**Two things need a human before it is switched on:**

1. **Chase or Aron signing off the wording.** The transcript is in the 2026-08-27 journal entry and in the file's own `NOTE` header.
2. **The "bush hogging" question.** Three independent transcription models each heard *bush hogging* with high confidence, and the VTT records what he actually said. **The site's own copy says "brush hogging" over 100 times, against two instances of "bush".** Captions should match speech, so this is not a defect — but it is a visible inconsistency the moment captions are enabled, and Aron has decided it stays as spoken.

Enabling it is one line in `initializeIntroVideo()`: `captionTrack.default = true`. Clearing `businessConfig.introVideoCaptions` removes the track entirely.

### 55. The poster and captions are welded to *this* take of the video
Item 13 notes a better recording is expected. When it arrives, `businessConfig.introVideoUrl` is a one-line swap **but two other artifacts silently become wrong**: `chaseIntro.poster.webp` is frame t=21.30s of the current take, and every timestamp in `chaseIntro.en.vtt` refers to the current take's audio. Neither will error; both will simply be subtly incorrect — a poster showing a different shirt, captions drifting against speech.

**Regenerate both, and re-run `node _qa/runAll.js --video`**, which asserts the cue count, the first and last cue text, that no cue runs past the end of the video, and that the poster path resolves. Archive the new master beside the old one rather than overwriting it.
